...
class MangaListMapper @Inject constructor(
	...
	private fun toDetailedListModel(
		...
		override = override,
		counter = badges.counter,
		progress = resolveProgress(manga, badges.progress),
	...
	private fun toGridModel(
		...
		isHiddenFromMain = settings.incognitoMode == IncognitoMode.HIDDEN_HISTORY &&
	...
	private fun resolveProgress(manga: Manga, progress: ReadingProgress?): ReadingProgress? {
		if (progress == null) {
			return null
		}
		if (settings.incognitoMode == IncognitoMode.HIDDEN_HISTORY) {
			if (manga.isNsfw() || settings.isPrivateHistory(manga.id)) {
				return null
			}
		} else if (settings.incognitoMode.isActive && manga.isNsfw()) {
			return null
		}
		return progress
	}
