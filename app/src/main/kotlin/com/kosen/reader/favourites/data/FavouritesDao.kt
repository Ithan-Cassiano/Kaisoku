...
abstract class FavouritesDao : MangaQueryBuilder.ConditionCallback {
	...
	@Query(
		...
		WHERE favourites.manga_id = :mangaId AND favourites.deleted_at = 0
		AND favourites.category_id = :categoryId
		)""",
	)
	abstract suspend fun isMangaInCategory(mangaId: Long, categoryId: Long): Boolean
	...
	@Query("SELECT DISTINCT manga_id FROM favourites WHERE deleted_at = 0 AND manga_id IN (:mangaIds)")
	abstract suspend fun findExistingMangaIds(mangaIds: LongArray): LongArray
