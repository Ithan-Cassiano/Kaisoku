...
import com.kosen.reader.favourites.data.toFavouriteCategory
import com.kosen.reader.favourites.data.toMangaList
import com.kosen.reader.favourites.domain.model.Cover
...
class FavouritesRepository @Inject constructor(
	...
	suspend fun isInReadLaterCategory(mangaId: Long): Boolean {
		val categoryId = db.getFavouriteCategoriesDao()
			.findCategoryIdByTitle(context.getString(R.string.read_later))
			?: return false
		return db.getFavouritesDao().isMangaInCategory(mangaId, categoryId)
	}
	...
	suspend fun getFavoriteIds(mangaIds: Collection<Long>): Set<Long> {
		...
		return db.getFavouritesDao().findExistingMangaIds(mangaIds.toLongArray()).toSet()
	}
