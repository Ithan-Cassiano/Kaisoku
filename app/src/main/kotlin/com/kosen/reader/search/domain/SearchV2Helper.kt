...
import com.kosen.reader.core.util.ext.contains
...
private const val MAX_RESULTS_TITLE = 12
private const val MAX_RESULTS_DEFAULT = 24
...
class SearchV2Helper @AssistedInject constructor(
	...
	private fun MutableList<Manga>.trimToMaxResults(kind: SearchKind) {
		val max = when (kind) {
			SearchKind.TITLE -> MAX_RESULTS_TITLE
			else -> MAX_RESULTS_DEFAULT
		}
		if (size > max) {
			subList(max, size).clear()
		}
	}
