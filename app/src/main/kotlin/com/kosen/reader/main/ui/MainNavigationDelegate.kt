...
import com.kosen.reader.bookmarks.ui.AllBookmarksFragment
...
import com.kosen.reader.core.util.ext.buildBundle
import com.kosen.reader.core.util.ext.setContentDescriptionAndTooltip
import com.kosen.reader.core.util.ext.smoothScrollToTop
import com.kosen.reader.databinding.NavigationRailFabBinding
...
import com.kosen.reader.history.ui.HistoryHomeFragment
...
import com.kosen.reader.local.ui.LocalListFragment
import com.kosen.reader.suggestions.ui.SuggestionsFragment
import com.kosen.reader.tracker.ui.feed.FeedFragment
import com.kosen.reader.tracker.ui.updates.UpdatesFragment
...
class MainNavigationDelegate(
	...
	fun openHistoryTab(suppressReselectAction: Boolean = false) {
		val itemId = R.id.nav_history
		if (navBar.selectedItemId != itemId) {
			navBar.selectedItemId = itemId
		}
		onNavigationItemSelected(itemId, suppressReselectAction)
	}
	...
	private fun onNavigationItemSelected(
		@IdRes itemId: Int,
		suppressReselectAction: Boolean = false,
	): Boolean {
		val newFragment = when (itemId) {
			R.id.nav_history -> HistoryHomeFragment::class.java
			...
			R.id.nav_suggestions -> resolveFragmentClass(
				releaseClass = SuggestionsFragment::class.java,
				debugClassName = "com.kosen.reader.suggestions.ui.DevSuggestionsFragment",
			)
			R.id.nav_bookmarks -> AllBookmarksFragment::class.java
			R.id.nav_updated -> UpdatesFragment::class.java
			else -> return false
		}
		if (!setPrimaryFragment(newFragment) && !suppressReselectAction) {
	...
	private fun getItemId(fragment: Fragment) = when (fragment) {
		is HistoryListFragment, is HistoryHomeUiHost -> R.id.nav_history
	...
	private fun setPrimaryFragment(fragmentClass: Class<out Fragment>): Boolean {
		...
		exitPrivateHistoryIfLeavingHistory(fragmentClass)
	...
	private fun exitPrivateHistoryIfLeavingHistory(newFragmentClass: Class<out Fragment>) {
		if (HistoryListFragment::class.java.isAssignableFrom(newFragmentClass)) {
			return
		}
		(primaryFragment as? HistoryListFragment)?.exitPrivateHistoryView()
	}
	...
	companion object {
		...
		private fun resolveFragmentClass(
			releaseClass: Class<out Fragment>,
			debugClassName: String? = null,
			redesignClassName: String? = null,
		): Class<out Fragment> {
			val candidate = when {
				BuildConfig.UX_REDESIGN -> redesignClassName
				BuildConfig.DEBUG -> debugClassName
				else -> null
			} ?: return releaseClass
			return runCatching {
				Class.forName(candidate).asSubclass(Fragment::class.java)
			}.getOrElse { releaseClass }
		}
	}
}
