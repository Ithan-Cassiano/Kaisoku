...
import com.kosen.reader.core.network.DoHProvider
import com.kosen.reader.core.util.ext.connectivityManager
import com.kosen.reader.core.util.ext.getEnumValue
import com.kosen.reader.core.util.ext.observeChanges
import com.kosen.reader.core.util.ext.putAll
import com.kosen.reader.core.util.ext.putEnumValue
import com.kosen.reader.core.util.ext.takeIfReadable
import com.kosen.reader.core.util.ext.toUriOrNull
...
import com.kosen.reader.parsers.util.find
...
import com.kosen.reader.reader.domain.ReaderColorFilter
import java.io.File
...
class AppSettings @Inject constructor(@ApplicationContext context: Context) {
	...
	var isNsfwFilterStrict: Boolean
		get() = prefs.getBoolean(KEY_NSFW_FILTER_STRICT, false)
		set(value) = prefs.edit { putBoolean(KEY_NSFW_FILTER_STRICT, value) }
	...
	val isTrackerSmartNotificationsEnabled: Boolean
		get() = prefs.getBoolean(KEY_TRACKER_SMART_NOTIFICATIONS, true)

	var isPrivateHistoryLockEnabled: Boolean
		get() = prefs.getBoolean(KEY_PRIVATE_HISTORY_LOCK, false)
		set(value) = prefs.edit { putBoolean(KEY_PRIVATE_HISTORY_LOCK, value) }

	var isPrivateHistoryBiometricEnabled: Boolean
		get() = prefs.getBoolean(KEY_PRIVATE_HISTORY_LOCK_BIO, true)
		set(value) = prefs.edit { putBoolean(KEY_PRIVATE_HISTORY_LOCK_BIO, value) }

	var isPrivateHistoryVolumeShortcutEnabled: Boolean
		get() = prefs.getBoolean(KEY_PRIVATE_HISTORY_VOLUME_SHORTCUT, false)
		set(value) = prefs.edit { putBoolean(KEY_PRIVATE_HISTORY_VOLUME_SHORTCUT, value) }

	var privateHistoryPinHash: String?
		get() = prefs.getString(KEY_PRIVATE_HISTORY_PIN, null)
		set(value) = prefs.edit {
			if (value != null) putString(KEY_PRIVATE_HISTORY_PIN, value) else remove(KEY_PRIVATE_HISTORY_PIN)
		}

	var isPrivateHistoryPinNumeric: Boolean
		get() = prefs.getBoolean(KEY_PRIVATE_HISTORY_PIN_NUMERIC, true)
		set(value) = prefs.edit { putBoolean(KEY_PRIVATE_HISTORY_PIN_NUMERIC, value) }

	fun hasPrivateHistoryPin(): Boolean = !privateHistoryPinHash.isNullOrEmpty()

	var devGithubToken: String?
		get() = prefs.getString(KEY_DEV_GITHUB_TOKEN, null)?.trim()?.nullIfEmpty()
		set(value) = prefs.edit {
			if (value.isNullOrBlank()) remove(KEY_DEV_GITHUB_TOKEN) else putString(KEY_DEV_GITHUB_TOKEN, value.trim())
		}

	var isDevTabBackgroundEnabled: Boolean
		get() = prefs.getBoolean(KEY_DEV_TAB_BACKGROUND, false)
		set(value) = prefs.edit { putBoolean(KEY_DEV_TAB_BACKGROUND, value) }

	var isReaderEdgeChapterNavEnabled: Boolean
		get() = prefs.getBoolean(KEY_READER_EDGE_CHAPTER_NAV, false)
		set(value) = prefs.edit { putBoolean(KEY_READER_EDGE_CHAPTER_NAV, value) }

	var sourcePreferencesByTypeJson: String?
		get() = prefs.getString(KEY_SOURCE_PREF_BY_TYPE, null)
		set(value) = prefs.edit {
			if (value != null) putString(KEY_SOURCE_PREF_BY_TYPE, value) else remove(KEY_SOURCE_PREF_BY_TYPE)
		}
	...
	var searchScope: String
		get() = prefs.getString(KEY_SEARCH_SCOPE, "TITLE") ?: "TITLE"
		set(value) = prefs.edit { putString(KEY_SEARCH_SCOPE, value) }

	var isNsfwCoverBlurEnabled: Boolean
		get() = prefs.getBoolean(KEY_NSFW_COVER_BLUR, true)
		set(value) = prefs.edit { putBoolean(KEY_NSFW_COVER_BLUR, value) }
	...
	var isIncognitoModeEnabled: Boolean
		get() = incognitoMode.isActive
		set(value) {
			incognitoMode = if (value) IncognitoMode.NO_HISTORY else IncognitoMode.DISABLED
		}

	var incognitoMode: IncognitoMode
		get() {
			if (prefs.contains(KEY_INCOGNITO_MODE_TYPE)) {
				return prefs.getEnumValue(KEY_INCOGNITO_MODE_TYPE, IncognitoMode.DISABLED)
			}
			return if (prefs.getBoolean(KEY_INCOGNITO_MODE, false)) {
				IncognitoMode.NO_HISTORY
			} else {
				IncognitoMode.DISABLED
			}
		}
		set(value) = prefs.edit {
			putEnumValue(KEY_INCOGNITO_MODE_TYPE, value)
			putBoolean(KEY_INCOGNITO_MODE, value.isActive)
		}

	var privateHistoryMangaIds: Set<Long>
		get() = prefs.getStringSet(KEY_PRIVATE_HISTORY_IDS, emptySet()).orEmpty()
			.mapNotNullToSet { it.toLongOrNull() }
		private set(value) = prefs.edit {
			putStringSet(KEY_PRIVATE_HISTORY_IDS, value.mapToSet { it.toString() })
		}

	fun markPrivateHistory(mangaId: Long) {
		val ids = privateHistoryMangaIds.toMutableSet()
		if (ids.add(mangaId)) {
			privateHistoryMangaIds = ids
		}
	}

	fun unmarkPrivateHistory(mangaId: Long) {
		val ids = privateHistoryMangaIds.toMutableSet()
	...
	fun isPrivateHistory(mangaId: Long): Boolean = mangaId in privateHistoryMangaIds

	fun clearPrivateHistoryIds() {
		privateHistoryMangaIds = emptySet()
	}

	private var privateHistoryUnlockSession: String? = null

	fun isPrivateHistoryUnlocked(sessionId: String): Boolean =
		!isPrivateHistoryLockEnabled || privateHistoryUnlockSession == sessionId

	fun markPrivateHistoryUnlocked(sessionId: String) {
		privateHistoryUnlockSession = sessionId
	}

	fun lockPrivateHistory(sessionId: String) {
		if (privateHistoryUnlockSession == sessionId) {
			privateHistoryUnlockSession = null
		}
	}

	fun lockAllPrivateHistorySessions() {
		privateHistoryUnlockSession = null
	}
	...
	val trackSources: Set<String>
		get() = prefs.getStringSet(KEY_TRACK_SOURCES, null) ?: setOf(TRACK_FAVOURITES, TRACK_HISTORY)
	...
	var isBrokenSourcesCleanupDone: Boolean
		get() = prefs.getBoolean(KEY_BROKEN_SOURCES_CLEANUP_DONE, false)
		set(value) = prefs.edit { putBoolean(KEY_BROKEN_SOURCES_CLEANUP_DONE, value) }

	var isSuggestionsBootstrapDone: Boolean
		get() = prefs.getBoolean(KEY_SUGGESTIONS_BOOTSTRAP_DONE, false) ||
			prefs.getBoolean(KEY_DEBUG_SUGGESTIONS_BOOTSTRAP_DONE, false)
		set(value) = prefs.edit { putBoolean(KEY_SUGGESTIONS_BOOTSTRAP_DONE, value) }

	var isPrivateHistoryReconcileDone: Boolean
		get() = prefs.getBoolean(KEY_PRIVATE_HISTORY_RECONCILE_DONE, false)
		set(value) = prefs.edit { putBoolean(KEY_PRIVATE_HISTORY_RECONCILE_DONE, value) }

	@Deprecated("Use isSuggestionsBootstrapDone", ReplaceWith("isSuggestionsBootstrapDone"))
	var isDebugSuggestionsBootstrapDone: Boolean
		get() = isSuggestionsBootstrapDone
		set(value) {
			isSuggestionsBootstrapDone = value
		}
	...
	var isPeriodicalBackupEnabled: Boolean
		get() = prefs.getBoolean(KEY_BACKUP_PERIODICAL_ENABLED, false)
		set(value) = prefs.edit { putBoolean(KEY_BACKUP_PERIODICAL_ENABLED, value) }
	...
	fun isIncognitoModeEnabled(isNsfw: Boolean): Boolean {
		if (incognitoMode.isActive) {
			return true
		}
		return isNsfw && incognitoModeForNsfw == TriStateOption.ENABLED
	}

	fun shouldSkipHistory(manga: com.kosen.reader.parsers.model.Manga): Boolean {
		return when (incognitoMode) {
			IncognitoMode.NO_HISTORY -> true
			IncognitoMode.HIDDEN_HISTORY -> false
			IncognitoMode.DISABLED -> manga.isNsfw() && incognitoModeForNsfw == TriStateOption.ENABLED
		}
	}
	...
	companion object {
		...
		const val KEY_TRACKER_SMART_NOTIFICATIONS = "tracker_smart_notifications"
		const val KEY_PRIVATE_HISTORY_LOCK = "private_history_lock"
		const val KEY_PRIVATE_HISTORY_LOCK_BIO = "private_history_lock_bio"
		const val KEY_PRIVATE_HISTORY_PIN = "private_history_pin"
		const val KEY_PRIVATE_HISTORY_PIN_NUMERIC = "private_history_pin_numeric"
		const val KEY_PRIVATE_HISTORY_CHANGE_PIN = "private_history_change_pin"
		const val KEY_PRIVATE_HISTORY_VOLUME_SHORTCUT = "private_history_volume_shortcut"
		const val KEY_DEV_GITHUB_TOKEN = "dev_github_token"
		const val KEY_DEV_TAB_BACKGROUND = "dev_tab_background"
		const val KEY_BACKUP_PERIODICAL_OUTPUT_DRIVE = "backup_periodic_output_drive"
		const val KEY_BACKUP_PERIODICAL_OUTPUT_LOCAL = "backup_periodic_output_local"
		const val KEY_READER_EDGE_CHAPTER_NAV = "reader_edge_chapter_nav"
		const val KEY_SOURCE_PREF_BY_TYPE = "source_pref_by_type"
		...
		const val KEY_BACKUP_DRIVE_SETUP = "backup_drive_setup"
		const val KEY_PRIVATE_HISTORY_EXPORT = "private_history_export"
		const val KEY_PRIVATE_HISTORY_CLEAR = "private_history_clear"
		const val KEY_PRIVATE_HISTORY_CATEGORY = "private_history_category"
		...
		const val KEY_INCOGNITO_MODE_TYPE = "incognito_mode_type"
		const val KEY_PRIVATE_HISTORY_IDS = "private_history_ids"
		...
		const val KEY_NSFW_FILTER_STRICT = "nsfw_filter_strict"
		...
		const val KEY_NSFW_COVER_BLUR = "nsfw_cover_blur"
		...
		const val KEY_BROKEN_SOURCES_CLEANUP_DONE = "broken_sources_cleanup_done"
		const val KEY_PRIVATE_HISTORY_RECONCILE_DONE = "private_history_reconcile_done"
		const val KEY_SUGGESTIONS_BOOTSTRAP_DONE = "suggestions_bootstrap_done"
		const val KEY_DEBUG_SUGGESTIONS_BOOTSTRAP_DONE = "debug_suggestions_bootstrap_done"
