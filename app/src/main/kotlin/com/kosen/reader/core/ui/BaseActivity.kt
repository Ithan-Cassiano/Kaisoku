...
import com.kosen.reader.core.ui.util.ActionModeDelegate
import com.kosen.reader.core.util.ext.isWebViewUnavailable
...
abstract class BaseActivity<B : ViewBinding> :
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		if (applyColorSchemeTheme) {
			if (BuildConfig.UX_REDESIGN) {
				// Kosen UX forces its own bold palette regardless of the user's theme/Monet choice.
				setTheme(R.style.ThemeOverlay_Kosen_KosenUx)
			} else {
				setTheme(settings.colorScheme.styleResId)
			}
		if (isAmoledTheme && !BuildConfig.UX_REDESIGN) {
				setTheme(R.style.ThemeOverlay_Kosen_Amoled)
			}
		}
	...
	override fun dispatchKeyEvent(event: KeyEvent): Boolean {
		if (event.action == KeyEvent.ACTION_DOWN &&
			entryPoint.privateHistoryVolumeShortcut.handleKeyEvent(event.keyCode)
		) {
			return true
		}
