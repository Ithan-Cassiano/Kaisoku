package com.kosen.reader.core.crash
...
internal object CrashReportFormatter {
	...
	private fun buildIssueUrl(
		...
		val baseUrl = context.getString(R.string.url_error_report)
			.trim()
			.removeSuffix("/choose")
			.ifEmpty { "mailto:?subject=Kosen%20Bug%20Report" }
		if (baseUrl.startsWith("mailto:", ignoreCase = true)) {
			return buildMailtoReportUrl(baseUrl, title, body)
		}
	...
	private fun buildMailtoReportUrl(baseUrl: String, title: String, body: String): String {
		val parsed = Uri.parse(baseUrl)
		val subject = title.ifBlank { parsed.getQueryParameter("subject") ?: "Kosen Bug Report" }
