package com.kosen.reader.core.parser.mihon
...
class MihonMangaRepository(
	...
	override suspend fun getList(offset: Int, order: SortOrder?, filter: MangaListFilter?): List<Manga> = withContext(Dispatchers.IO) {
		runCatching {
			...
			mangas.mangas.map { it.toKosenManga() }
	...
	override suspend fun getDetailsImpl(manga: Manga): Manga = withContext(Dispatchers.IO) {
		...
		val chapters = loadChapters(seed, details).orderForReading().mapIndexed { index, chapter ->
			chapter.toKosenChapter(fallbackNumber = (index + 1).toFloat())
		}
		details.toKosenManga(chapters = chapters).copy(id = manga.id)
	}
	...
	override fun createPageRequest(pageUrl: String, page: MangaPage): Request {
		...
		return (request ?: Request.Builder()
			...
			.build())
			.newBuilder()
			.tag(com.kosen.reader.parsers.model.MangaSource::class.java, page.source)
			.build()
	}

	private fun SManga.toKosenManga(chapters: List<MangaChapter>? = null): Manga {
		val rawUrl = safeUrl()
	...
	private fun SChapter.toKosenChapter(fallbackNumber: Float): MangaChapter {
		val rawUrl = safeUrl()
		return MangaChapter(
	...
	private fun Page.toKosenPage(index: Int): MangaPage {
	...
	private fun List<SChapter>.orderForReading(): List<SChapter> {
		if (size <= 1) {
			return this
		}
		val withValidNumber = count { it.safeChapterNumber() > 0f }
		if (withValidNumber >= (size + 1) / 2) {
			return sortedWith(compareBy({ it.safeChapterNumber() }, { it.safeUploadDate() }, { it.safeName() }))
		}
		return asReversed()
	}
