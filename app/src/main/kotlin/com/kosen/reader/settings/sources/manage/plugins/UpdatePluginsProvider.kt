...
class UpdatePluginsProvider @Inject constructor(
	...
	suspend fun runAutoUpdate(settings: AppSettings) {
		...
		mutex.withLock {
			withContext(Dispatchers.Default) {
				...
				if (now - settings.lastAutoPlugins < autoUpdateCooldownMs()) return@withContext
	...
	companion object {
		...
		private const val DEBUG_COOLDOWN = 3_600_000L // 1h

		private fun autoUpdateCooldownMs(): Long =
			if (BuildConfig.DEBUG) DEBUG_COOLDOWN else COOLDOWN
