...
import com.kosen.reader.databinding.ActivitySetupProtectBinding

private const val MIN_PASSWORD_LENGTH = 4

@AndroidEntryPoint
class PrivateHistoryPinSetupActivity :
	...
	private val viewModel by viewModels<PrivateHistoryPinSetupViewModel>()

	override fun onCreate(savedInstanceState: Bundle?) {
		super.onCreate(savedInstanceState)
		window.addFlags(WindowManager.LayoutParams.FLAG_SECURE)
		setContentView(ActivitySetupProtectBinding.inflate(layoutInflater))
		viewBinding.textViewTitle.setText(R.string.private_history_pin_setup_title)
		viewBinding.textViewSubtitle.setText(R.string.private_history_pin_setup_subtitle)
		...
		viewModel.isSecondStep.observe(this, ::onStepChanged)
	...
	override fun onClick(v: View) {
		when (v.id) {
			...
			R.id.button_next -> viewModel.onNextClick(
				viewBinding.editPassword.text?.toString() ?: return,
			)
		}
	}
	...
	private fun onStepChanged(isSecondStep: Boolean) {
		...
		viewBinding.switchBiometric.isVisible = isSecondStep
		if (isSecondStep) {
	...
	companion object {
		fun newIntent(context: Context): Intent =
			Intent(context, PrivateHistoryPinSetupActivity::class.java)
	}
}
