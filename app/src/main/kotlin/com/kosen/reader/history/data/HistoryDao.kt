...
import com.kosen.reader.list.domain.ReadingProgress.Companion.PROGRESS_COMPLETED
...
abstract class HistoryDao : MangaQueryBuilder.ConditionCallback {
	...
	@Query(
		"""SELECT history.manga_id FROM history
		LEFT JOIN manga ON manga.manga_id = history.manga_id
		WHERE history.deleted_at = 0 AND manga.source = :source""",
	)
	abstract suspend fun findMangaIdsBySource(source: String): List<Long>

	@Query(
		"""SELECT DISTINCT history.manga_id FROM history
		LEFT JOIN manga_tags ON manga_tags.manga_id = history.manga_id
		LEFT JOIN tags ON tags.tag_id = manga_tags.tag_id
		WHERE history.deleted_at = 0 AND tags.title LIKE :tagQuery""",
	)
	abstract suspend fun findMangaIdsByTagTitle(tagQuery: String): List<Long>
	...
	override fun getCondition(option: ListFilterOption): String? = when (option) {
		...
		ListFilterOption.Macro.WEEKLY_READ -> {
			val weekAgo = System.currentTimeMillis() - 7L * 24 * 60 * 60 * 1000
			"history.updated_at >= $weekAgo"
		}
		else -> null
	}
}
