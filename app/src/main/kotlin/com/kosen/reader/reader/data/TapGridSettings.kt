package com.kosen.reader.reader.data
...
import com.kosen.reader.reader.ui.tapgrid.TapAction
import javax.inject.Inject

@Reusable
class TapGridSettings @Inject constructor(
	...
	fun getTapAction(area: TapGridArea, isLongTap: Boolean): TapAction? {
		...
		val action = prefs.getEnumValue(key, TapAction::class.java)
		if (!isLongTap && appSettings.isReaderEdgeChapterNavEnabled) {
			when (area) {
				TapGridArea.CENTER_LEFT -> return TapAction.CHAPTER_PREV
				TapGridArea.CENTER_RIGHT -> return TapAction.CHAPTER_NEXT
				TapGridArea.CENTER -> return TapAction.TOGGLE_UI
				else -> Unit
			}
		}
		return action
