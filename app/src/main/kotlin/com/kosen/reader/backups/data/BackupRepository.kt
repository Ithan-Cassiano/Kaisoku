package com.kosen.reader.backups.data
...
import com.kosen.reader.backups.data.model.BackupIndex
import com.kosen.reader.backups.data.model.BookmarkBackup
import com.kosen.reader.backups.data.model.CategoryBackup
import com.kosen.reader.backups.data.model.FavouriteBackup
import com.kosen.reader.backups.data.model.HistoryBackup
import com.kosen.reader.backups.data.model.MangaBackup
import com.kosen.reader.backups.data.model.ScrobblingBackup
import com.kosen.reader.backups.data.model.SourceBackup
import com.kosen.reader.backups.data.model.StatisticBackup
import com.kosen.reader.backups.domain.BackupSection
import com.kosen.reader.core.db.MangaDatabase
import com.kosen.reader.core.db.migrations.MangaIdentityMerge
import com.kosen.reader.core.prefs.AppSettings
import com.kosen.reader.core.util.CompositeResult
import com.kosen.reader.core.util.progress.Progress
...
class BackupRepository @Inject constructor(
    ...
    suspend fun createBackup(
        ...
        for (section in BackupSection.entries) {
            when (section) {
                ...
                BackupSection.PRIVATE_HISTORY -> output.writeString(
                    section = BackupSection.PRIVATE_HISTORY,
                    data = dumpPrivateHistorySnapshot(),
                )
            }
        ...
        while (entry != null) {
            ...
            if (section in sections) {
                result += when (section) {
                    ...
                    BackupSection.PRIVATE_HISTORY -> input.readString().let {
                        restorePrivateHistorySnapshot(it)
                        CompositeResult.success()
                    }
    ...
    private suspend fun dumpPrivateHistorySnapshot(): String {
        val ids = settings.privateHistoryMangaIds
        val root = JSONObject()
        root.put("incognito_mode", settings.incognitoMode.name)
        root.put("ids", JSONArray(ids.toList()))
        val entries = JSONArray()
        for (id in ids) {
            val history = database.getHistoryDao().find(id) ?: continue
            val manga = database.getMangaDao().find(id)?.manga ?: continue
            entries.put(
                JSONObject()
                    .put("id", id)
                    .put("title", manga.title)
                    .put("source", manga.source)
                    .put("percent", history.percent.toDouble()),
            )
        }
    ...
    private fun restorePrivateHistorySnapshot(raw: String) {
        val payload = JSONArray(raw).optJSONObject(0) ?: return
        payload.optString("incognito_mode").takeIf { it.isNotBlank() }?.let { name ->
            runCatching { IncognitoMode.valueOf(name) }.getOrNull()?.let { mode ->
                settings.incognitoMode = mode
            }
        }
        val idsJson = payload.optJSONArray("ids") ?: return
        settings.clearPrivateHistoryIds()
        for (index in 0 until idsJson.length()) {
            settings.markPrivateHistory(idsJson.getLong(index))
        }
    }
