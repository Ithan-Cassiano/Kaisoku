...
import com.kosen.reader.core.util.ext.toInstantOrNull
...
class TrackingRepository @Inject constructor(
	...
	suspend fun getNewChaptersCountMap(mangaIds: Collection<Long>): Map<Long, Int> {
		if (mangaIds.isEmpty()) {
			return emptyMap()
		}
		val tracks = db.getTracksDao().findAllByIds(mangaIds.toLongArray())
		return HashMap<Long, Int>(tracks.size).apply {
			for (track in tracks) {
				this[track.mangaId] = track.newChapters
			}
		}
	}
	...
	suspend fun updateTracks() = db.withTransaction {
		...
		val size = ids.size
		// history — always tracked so chapter releases appear in the feed
