package com.kosen.reader.tracker.work
...
import com.kosen.reader.core.prefs.TrackerDownloadStrategy
...
import com.kosen.reader.parsers.util.toIntUp
...
import com.kosen.reader.tracker.domain.model.MangaUpdates
import com.kosen.reader.tracker.work.TrackerNotificationHelper.NotificationInfo
...
class TrackWorker @AssistedInject constructor(
	...
	private val smartNotificationFilter: SmartTrackerNotificationFilter,
	...
	private suspend fun checkUpdatesAsync(tracks: List<MangaTracking>): List<NotificationInfo> {
		...
		val notifications = ArrayList<NotificationInfo>(tracks.size)
		channelFlow {
			for (track in tracks) {
				launch {
			...
			when (it) {
				...
				is MangaUpdates.Success -> {
					processDownload(it)
					if (it.isValid && it.isNotEmpty() && smartNotificationFilter.shouldNotify(it.manga)) {
